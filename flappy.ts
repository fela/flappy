...
class Game {
    ...
    bird: PIXI.Sprite = undefined
    speedY: number = undefined
    started: boolean = undefined
    gameOver: boolean = undefined
    birdTextureCounter: number = undefined
    pipes: PIXI.Graphics = undefined
    holeStart: number = undefined
    gameBox: PIXI.Container = new PIXI.Container()
    ...
    checkGameOver() {
        if (this.gameOver) {
            ...
            const outOfBound = (
                this.bird.y - BIRD_SIZE/2 + MARGIN < 0 || this.bird.y + BIRD_SIZE/2 - MARGIN > HEIGHT
            )

            const collided = ( 
                (
                    Math.abs(this.pipes.x + PIPES_WIDTH/2 - this.bird.x) < BIRD_SIZE/2 + PIPES_WIDTH/2 - MARGIN
                ) && (
                    this.bird.y - BIRD_SIZE/2 + MARGIN < this.holeStart ||
